...
class Ranged(Weapon):
    ...
    def summary(self) -> str:
        return "\n".join([
            ...
            *(f"{f'{dt.upper()}:':<25} {f'{self.base_damage_dist.get(dt):.2f}':<7} -> {self.moded_damage_dist.get(dt):.2f}" for dt, _ in self.moded_damage_dist),
            ...
            *(f"{f'{dt.upper()}:':<25} {f'{self.base_explosion_damage_dist.get(dt):.2f}':<7} -> {self.moded_explosion_damage_dist.get(dt):.2f}" for dt, _ in self.moded_explosion_damage_dist),
