...
class RangedCalculator(WeaponCalculator):
    ...
    def _compute_effective_stats(self) -> None:
        ...
        self.effective.reload_speed = self.moded.reload_speed + (0 if not self.data.context.get("is_battery", False) else float("inf") if self.moded.recharge_rate <= 0 else self.moded.magazine_capacity / self.moded.recharge_rate)
        self.effective.recharge_rate = self.moded.recharge_rate
        self.effective.ammo_efficiency = 1 - (1 - self.moded.ammo_efficiency) / (2 if self.data.context.get("is_beam", False) else 1)
