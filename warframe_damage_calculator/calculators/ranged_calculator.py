...
class RangedCalculator(WeaponCalculator):
    def _compute_modded_stats(self) -> None:
        ...
        self.modded.explosion_damage = self.modded.base_damage * self.base.explosion_damage.apply(build.damage).combine().sorted()
    ...
    def _compute_effective_stats(self) -> None:
        ...
        self.effective.explosion_damage = self.modded.explosion_damage
        ...
        self.effective.reload_speed = self.modded.reload_speed + (0 if not self.weapon.data.context.get("is_battery", False) else float("inf") if self.modded.recharge_rate <= 0 else self.modded.magazine_capacity / self.modded.recharge_rate)
        self.effective.recharge_rate = self.modded.recharge_rate
        self.effective.ammo_efficiency = 1 - (1 - self.modded.ammo_efficiency) / (2 if self.weapon.data.context.get("is_beam", False) else 1)
    ...
    def _compute_average_stats(self) -> None:
        ...
        self.average.flat_dph = (self.effective.damage.total_damage() * self.effective.multishot + self.effective.explosion_damage.total_damage()) * self.effective.faction_damage * self.average.crit_multiplier
        self.average.flat_weakpoint_dph = (self.effective.damage.total_damage() * self.effective.multishot * self.effective.weakpoint_damage * self.average.weakpoint_crit_multiplier + self.effective.explosion_damage.total_damage() * self.average.crit_multiplier) * self.effective.faction_damage
