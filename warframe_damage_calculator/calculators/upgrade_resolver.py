...
class UpgradeResolver:
    ...
    def _automatic_context(self, weapon: WeaponState, build: Build) -> dict[str, object]:
        ...
        context["sacrificial set"] = {
            "sacrificial pressure",
            "sacrificial steel",
        }.issubset(upgrade_names)
        return context
    ...
    def _merge(target: dict[str, Value], stat: str, value: Value) -> None:
        ...
        if current is None:
            ...
            try:
                ...
                raise TypeError(
                    f"Cannot merge values for upgrade stat {stat!r}"
                ) from None
    ...
    def resolve(self, weapon: WeaponState, build: Build) -> Build:
        ...
        for upgrade in build:
            ...
            use_defaults = not any(
                self._normalize(key) != "rank" for key in upgrade.context
            )
            ...
            if isinstance(rank, bool) or not isinstance(rank, int) or rank < 0:
                raise ValueError(
                    f"Invalid rank {rank!r} for upgrade {upgrade_name!r}; "
            ...
            multiplier = (
                1.0
                if upgrade.max_rank in {None, 0}
                else (rank + 1) / (upgrade.max_rank + 1)
            )
            ...
            for stat, (value, condition) in upgrade.stacking_stats.items():
                ...
                stack_count = context.get(
                    normalized_condition,
                    (upgrade.max_stacks or 0) if use_defaults else 0,
                )
                if (
                    isinstance(stack_count, bool)
                    or not isinstance(stack_count, int)
                    or stack_count < 0
                ):
                    raise ValueError(
                        f"Invalid stack count {stack_count!r} for condition "
                        f"{condition!r} on upgrade {upgrade_name!r}; "
                        "stack count must be a non-negative integer"
                    )
                ...
                if stack_count:
                    self._merge(
                        resolved,
                        stat,
                        self._scale(value, multiplier) * stack_count,
                    )
