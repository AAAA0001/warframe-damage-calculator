...
class RangedFormatter[TRangedState: RangedState](WeaponFormatter[TRangedState]):    
    ...
    def summary(self) -> str:
        return "\n".join([
            ...
            *(f"{f'{dt.upper()}:':<25} {f'{self.calculator.base.damage_dist.get(dt):.2f}':<7} -> {self.calculator.effective.damage_dist.get(dt):.2f}" for dt, _ in self.calculator.effective.damage_dist),
            ...
            *(f"{f'{dt.upper()}:':<25} {f'{self.calculator.base.explosion_damage_dist.get(dt):.2f}':<7} -> {self.calculator.effective.explosion_damage_dist.get(dt):.2f}" for dt, _ in self.calculator.effective.explosion_damage_dist),
