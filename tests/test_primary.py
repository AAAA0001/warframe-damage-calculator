...
class PrimaryTests(unittest.TestCase):
    def test_primary_specific_methods(self) -> None:
        ...
        weapon.configure(Build(Upgrade(primed_chamber=1.0, hunter_munitions=0.3, vigilante_bonus=0.05, internal_bleeding=0.7)))
        calc = weapon.calculate

        self.assertAlmostEqual(calc.effective.primed_chamber, 1.0)
        self.assertAlmostEqual(calc.effective.hunter_munitions, 0.3)
        self.assertAlmostEqual(calc.effective.vigilante_bonus, 0.05)
        self.assertAlmostEqual(calc.effective.internal_bleeding, 1.4)
    ...
    def test_primary_raw_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(primed_chamber=0.5, base_damage=0.2, crit_damage=0.25)))
        calc = weapon.calculate
    ...
    def test_primary_dot_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(hunter_munitions=0.3, internal_bleeding=0.5, primed_chamber=0.4)))
        calc = weapon.calculate
