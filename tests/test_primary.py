...
class PrimaryTests(unittest.TestCase):
    def test_primary_regression_snapshot_values(self) -> None:
        weapon = Primary(damage_dist=dist(impact=25.2, puncture=37.8, slash=27), fire_rate=1.42, reload_speed=3.00, magazine_capacity=20, multishot=6.00, crit_chance=0.30, crit_damage=2.80, status_chance=0.09, is_beam=True)
        weapon.configure(Build(Upgrade(damage_dist=dist(impact=-0.886), crit_damage=0.855, multishot=1.126, crit_chance=0.887), Upgrade(multishot=1.10 + 0.30 * 4), Upgrade(base_damage=2.40), Upgrade(hunter_munitions=0.30), Upgrade(damage_dist=dist(cold=1.65)), Upgrade(crit_damage=1.10), Upgrade(crit_chance=2.00), Upgrade(damage_dist=dist(toxin=0.60), status_chance=0.60), Upgrade(vigilante_bonus=0.05), Upgrade(base_damage=0.30 * 12, reload_speed=0.30), Upgrade(flat_crit_damage=1.20)))
        calc = weapon.calculate

        self.assertAlmostEqual(calc.effective.fire_rate, 1.42000000)
        self.assertAlmostEqual(calc.effective.reload_speed, 2.30769231)
        self.assertAlmostEqual(calc.effective.multishot, 26.55600000)
        self.assertAlmostEqual(calc.effective.crit_chance, 1.21610000)
        self.assertAlmostEqual(calc.effective.weakpoint_crit_chance, 1.21610000)
        self.assertAlmostEqual(calc.effective.status_chance, 0.14400000)
        self.assertAlmostEqual(calc.effective.hunter_munitions, 0.30000000)
        self.assertAlmostEqual(calc.effective.primed_chamber, 0.00000000)
        self.assertAlmostEqual(calc.effective.vigilante_bonus, 0.05000000)
        self.assertAlmostEqual(calc.effective.total_damage, 1891.20960000)
        self.assertAlmostEqual(calc.average_primed_chamber_multiplier, 1.00000000)
    ...
    def test_primary_raw_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(primed_chamber=0.5, base_damage=0.2, crit_damage=0.25, multishot=0.3, fire_rate=0.2, weakpoint_crit_chance=0.4)))
        calc = weapon.calculate
    ...
    def test_primary_dot_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(hunter_munitions=0.3, internal_bleeding=0.5, primed_chamber=0.4, status_damage=0.3, multishot=0.5, faction_damage=0.2)))
        calc = weapon.calculate

        self.assertAlmostEqual(calc.flat_dotph, 24649.31813760)
        self.assertAlmostEqual(calc.flat_dotps, 18146.73727921)
        ...
        weapon.configure(Build(Upgrade(hunter_munitions=0.3, internal_bleeding=0.6, primed_chamber=0.9, status_damage=0.4, multishot=0.45, faction_damage=0.35, crit_damage=0.5)))
        calc = weapon.calculate
