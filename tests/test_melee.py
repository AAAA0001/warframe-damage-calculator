...
class MeleeTests(unittest.TestCase):
    ...
    def test_melee_regression_snapshot_values(self) -> None:
        ...
        weapon.configure(Build(Upgrade(stats={'base_damage': 1.2, 'multiplicative_base_damage': 0.5, 'faction_damage': 0.4, 'crit_chance': 0.6, 'crit_damage': 0.9, 'flat_crit_damage': 0.4, 'status_chance': 0.35, 'status_damage': 0.25, 'attack_speed': 0.3, 'melee_duplicate': 0.35})))
        calc = weapon.stats
        self.assertAlmostEqual(calc.effective.attack_speed, 1.495)
        self.assertAlmostEqual(calc.effective.melee_duplicate, 0.35)
        self.assertAlmostEqual(calc.effective.melee_doughty, 0.0)
        self.assertAlmostEqual(calc.effective.crit_chance, 0.88)
        self.assertAlmostEqual(calc.effective.crit_damage, 4.39)
        self.assertAlmostEqual(calc.effective.status_chance, 0.6075)
        self.assertAlmostEqual(calc.effective.total_damage, 495.0)
        self.assertAlmostEqual(calc.average_melee_duplicate_multiplier, 1.308)
        self.assertAlmostEqual(calc.flat_dph, 3610.5477408)
    ...
    def test_melee_raw_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(stats={'base_damage': 0.25, 'multiplicative_base_damage': 0.4, 'faction_damage': 0.55, 'melee_duplicate': 0.1, 'flat_crit_damage': 0.25})))
        calc = weapon.stats
        ...
        weapon.configure(Build(Upgrade(stats={'base_damage': 0.4, 'multiplicative_base_damage': 0.2, 'faction_damage': 0.3, 'melee_duplicate': 0.25, 'crit_chance': 0.5, 'crit_damage': 0.6, 'flat_crit_damage': 0.15})))
        calc = weapon.stats
    ...
    def test_melee_dot_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(stats={'status_damage': 0.2, 'melee_duplicate': 0.15, 'crit_chance': 0.25, 'crit_damage': 0.4})))
        calc = weapon.stats
        self.assertAlmostEqual(calc.flat_dotph, 292.8515625)
        self.assertAlmostEqual(calc.flat_dotps, 351.421875)
        ...
        weapon.configure(Build(Upgrade(stats={'status_damage': 0.35, 'melee_duplicate': 0.1, 'melee_doughty': 0.2, 'crit_damage': 0.45, 'faction_damage': 0.2})))
        calc = weapon.stats
