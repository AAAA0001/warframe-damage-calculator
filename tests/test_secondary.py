...
from warframe_damage_calculator.models import Secondary, Upgrade
...
class SecondaryTests(unittest.TestCase):
    def test_secondary_specific_methods(self) -> None:
        ...
        self.assertAlmostEqual(disabled.average_secondary_enervate_bonus(), 0.0)
        self.assertAlmostEqual(disabled.average_weakpoint_secondary_enervate_bonus(), 0.0)

        enabled = Secondary(crit_chance=0.40)
        enabled.configure(Upgrade(secondary_enervate=4, weakpoint_crit_chance=1.2))

        self.assertAlmostEqual(enabled.average_secondary_enervate_bonus(), 0.7242222499496438)
        self.assertAlmostEqual(enabled.average_weakpoint_secondary_enervate_bonus(), 0.48690152992395375)
    ...
    def test_secondary_raw_damage_formulas(self) -> None:
        ...
        self.assertAlmostEqual(weapon.flat_dph(), 456.0)
        self.assertAlmostEqual(weapon.flat_dps(), 912.0)
    ...
    def test_secondary_dot_damage_formulas(self) -> None:
        ...
        self.assertAlmostEqual(weapon.flat_dotph(), 802.62)
        self.assertAlmostEqual(weapon.flat_dotps(), 1605.24)
