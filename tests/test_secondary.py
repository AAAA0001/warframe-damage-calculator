...
class SecondaryTests(unittest.TestCase):
    def test_secondary_specific_methods(self) -> None:
        ...
        disabled.configure(Build(Upgrade(secondary_enervate=0, weakpoint_crit_chance=0.5)))
        disabled_calc = disabled.calculate
        ...
        enabled.configure(Build(Upgrade(secondary_enervate=4, weakpoint_crit_chance=1.2)))
        enabled_calc = enabled.calculate
    ...
    def test_secondary_raw_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(base_damage=0.2, crit_damage=0.25)))
        calc = weapon.calculate

        self.assertAlmostEqual(calc.flat_dph, 456.0)
        self.assertAlmostEqual(calc.flat_dps, 977.1428571428571)
    ...
    def test_secondary_dot_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(internal_bleeding=0.8, secondary_encumber=0.12)))
        calc = weapon.calculate
