...
class SecondaryTests(unittest.TestCase):
    def test_secondary_regression_snapshot_values(self) -> None:
        ...
        weapon.configure(Build(Upgrade(base_damage=0.85, multiplicative_base_damage=0.6, crit_chance=1.1, crit_damage=0.75, flat_crit_damage=0.35, status_chance=0.45, status_damage=0.5, fire_rate=0.4, multishot=0.95, reload_speed=0.25, weakpoint_damage=0.9, internal_bleeding=0.7, secondary_enervate=5, secondary_encumber=0.2)))
        calc = weapon.calculate

        self.assertAlmostEqual(calc.effective.fire_rate, 4.48000000)
        self.assertAlmostEqual(calc.effective.reload_speed, 1.44000000)
        self.assertAlmostEqual(calc.effective.multishot, 4.29000000)
        self.assertAlmostEqual(calc.effective.crit_chance, 0.88200000)
        self.assertAlmostEqual(calc.effective.weakpoint_crit_chance, 0.88200000)
        self.assertAlmostEqual(calc.effective.secondary_enervate, 5.00000000)
        self.assertAlmostEqual(calc.effective.secondary_encumber, 0.20000000)
        self.assertAlmostEqual(calc.effective.internal_bleeding, 0.70000000)
    ...
    def test_secondary_raw_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(base_damage=0.2, crit_damage=0.25, multishot=0.4, fire_rate=0.2, secondary_enervate=3)))
        calc = weapon.calculate
        ...
        weapon.configure(Build(Upgrade(base_damage=0.45, crit_damage=0.35, multishot=0.55, fire_rate=0.3, secondary_enervate=4, faction_damage=0.15)))
        calc = weapon.calculate
    ...
    def test_secondary_dot_damage_formulas(self) -> None:
        ...
        weapon.configure(Build(Upgrade(internal_bleeding=0.9, secondary_encumber=0.18, status_damage=0.4, multishot=0.35, crit_damage=0.55, faction_damage=0.25, secondary_enervate=5)))
        calc = weapon.calculate
